...
When a keyword was detected in the user prompt, the extension SHALL send a user message at `agent_end`. The message SHALL be sent via `pi.sendUserMessage()`, which creates a real user message and triggers a new agent turn.
...
- **THEN** the extension SHALL call `pi.sendUserMessage` with the follow-up text
- **AND** the follow-up SHALL trigger a new agent turn immediately
